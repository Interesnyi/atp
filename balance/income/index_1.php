...
<html lang="en">
  <head>
    ...
    <title>ELDIR | Модуль Баланс | Приход </title>
...
<body>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="payment_method" class="form-select">
                            <option value="0">Выберите метод оплаты</option>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select id="payment_method" name="payment_method" class="form-select">
                            <option value="0">Выберите метод оплаты</option>
                            <option value="non-cash">Безналичный расчёт</option>
                            <option value="cash" selected>Наличный расчёт</option>                            
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="summa">Сумма платежа</label>
    ...
    <div class="container mt-4">
        <div class="row">
            <div class="col-md-12">
                <h1>ELDIR | Модуль Баланс | Приход </h1>
            </div>
        </div>
                ...
                <div class="table-responsive-sm">
                    <table id="myTable" class=" table table-bordered table-striped table-sm table-hover">
                        ...
                        <tbody>
                            ...
                            $query = "SELECT * FROM balance_income_expenses WHERE sign_of_calculation = 'income'";
                            ...
                            if(mysqli_num_rows($query_run) > 0)
                            {
                                ...
                                {   
                                    ?>
                                    <tr>
                                        <td><?= date("d.m.Y",strtotime($payment['date_operations'])) ?></td>
                                        <td><?= $payment['payment_method'] == 'non-cash' ? 'Безнал' : ($payment['payment_method'] == 'cash' ? 'Нал': '') ?></td>
                                        <td><?= $payment['summa'] ?></td>
                                        <td><?= get_text_type_payment($payment['type_payment']) ?></td>
                                        <td><?= $payment['purpose_payment'] ?></td>
                                        <td><?= $payment['comment'] ?></td>
                                        <td>
                                            <button type="button" value="<?=$payment['id'];?>" class="editInfoBtn btn btn-success btn-sm">Изменить</button>
                                            <button type="button" value="<?=$payment['id'];?>" class="deleteInfoBtn btn btn-danger btn-sm">Удалить</button>
                                        </td>
                                    </tr>
                                    <?php
                                }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(res.status == 404) {
                        ...
                        $('#sign_of_calculation').val(res.data.sign_of_calculation);
