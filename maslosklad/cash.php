...
<html lang="en">
  <head>
    ...
    <title>ELDIR | МаслоСклад | НАЛ</title>
                ...
                <div class="table-responsive-sm">
                    <table id="myTable" class=" table table-bordered table-striped table-sm table-hover">
                        ...
                        <tbody>
                            ...
                            $query = "SELECT s.id as id, sp.name as property_name, s.summa as summa, spt.name as type_name, s.count, s.date_operations, spp.name as place, s.comment,  IF(s.status > 0, 'Оплачен', 'Не оплачен') AS status, IF(s.type_pay > 0, 'безнал', 'нал') AS type_pay 
                                        FROM `maslosklad` s 
